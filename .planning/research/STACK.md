...
**Domain:** UCI chess engine with supervised-trained NNUE evaluation + classical alpha-beta search, Python-first, trained via PyTorch MPS on Apple Silicon M4
**Researched:** 2026-07-05
**Confidence:** HIGH on the core stack shape and library roles; MEDIUM on exact pinned versions (version numbers move fast — pin at project init and re-verify `torch.backends.mps.is_available()` on the actual macOS build)
...
**Explicitly NOT a library dependency:** the UCI protocol handler. Per the project decision, ANCE *is* the engine — hand-write the `uci`/`isready`/`ucinewgame`/`position`/`go`/`stop`/`quit` loop reading stdin and writing `bestmove` / `info` lines. `chess.engine` is the wrong tool for that (it is a client, not a server).
...
# Core
pip install chess            # python-chess 1.11.2 (imported as `chess`)
pip install torch            # stable wheel includes MPS; verify torch.backends.mps.is_available()
pip install numpy
...
brew install stockfish       # labeling + sparring
brew install lc0             # optional superhuman sparring (Metal backend default)
...
cd lichess-bot && pip install -r requirements.txt   # Python 3.10+
```

**Post-install sanity check (do this before pinning PyTorch):**
```bash
python - <<'PY'
import torch
print(torch.__version__, torch.backends.mps.is_available(), torch.backends.mps.is_built())
x = torch.randn(4, device="mps") @ torch.randn(4, device="mps")  # must not raise
PY
```
...
**If MPS is unavailable / buggy on the installed macOS:**
...
**If the Lichess `[%eval]` coverage is enough:**
- Skip a custom Stockfish labeling pass; parse `[%eval]` directly from PGN → faster path to first net.

**If you want faster/larger training later (out of scope this milestone):**
...
---
*Stack research for: Python NNUE + alpha-beta UCI chess engine on Apple Silicon M4*
*Researched: 2026-07-05*
